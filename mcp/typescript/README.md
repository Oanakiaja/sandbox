...
[![NPM Downloads](https://img.shields.io/npm/d18m/mcp-server-typescript)](https://www.npmjs.com/package/mcp-server-typescript)

A Model Context Protocol (MCP) server for typescript
...
You can also install the typescript MCP server using the VS Code CLI:

```bash
# For VS Code
code --add-mcp '{"name":"typescript","command":"npx","args":["mcp-server-typescript@latest"]}'
```
...
Go to `Cursor Settings` -> `MCP` -> `Add new MCP Server`. Name to your liking, `npx mcp-server-typescript`. You can also verify config or add command like arguments via clicking `Edit`.
...
npx mcp-server-typescript --port 8089
```
...
import { createServer } from 'mcp-server-typescript';
